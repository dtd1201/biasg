              ...
              @foreach ($rows as $row)
                <form action="{{ route(Request::segment(2) . '.destroy', $row->id) }}" method="POST"
                  ...
                  <tr class="valign-middle">
                    ...
                    <td>
                      {{-- $row->iorder --}}
                      <input data-url="{{ route('admin.loadOrderVeryModel',['table'=>'pages','id'=>$row->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $row->iorder?$row->iorder:0 }}" style="width:50px" />
                    </td>
                    <td>
                      {{ $row->updated_at }}
                    </td>
                    ...
                    <td class="wrap-load-active" data-url="{{ route('admin.pages.load.status',['id'=>$row->id]) }}">
                      @include('admin.components.load-change-status',['data'=>$row,'type'=>'page'])
                    </td>
                    <td>
