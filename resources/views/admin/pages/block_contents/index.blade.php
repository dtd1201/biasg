                ...
                @if ($row->parent_id == 0 || $row->parent_id == null)
                  <form action="{{ route(Request::segment(2) . '.destroy', $row->id) }}" method="POST"
                    ...
                    <tr class="valign-middle">
                      ...
                      <td>
                        {{-- $row->iorder --}}
                        <input data-url="{{ route('admin.loadOrderVeryModel',['table'=>'block_contents','id'=>$row->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $row->iorder?$row->iorder:0 }}" style="width:50px" />
                      </td>
                      <td>
                        {{ $row->updated_at }}
                      </td>
                      ...
                      <td class="wrap-load-active" data-url="{{ route('admin.block_contents.load.status',['id'=>$row->id]) }}">
                        @include('admin.components.load-change-status',['data'=>$row,'type'=>'Khối nội dung'])
                      </td>
                      <td>
                      ...
                      <form action="{{ route(Request::segment(2) . '.destroy', $sub->id) }}" method="POST"
                        ...
                        <tr class="valign-middle bg-gray-light">
                          ...
                          <td>
                            ...
                            - - - - <input data-url="  {{ route('admin.loadOrderVeryModel',['table'=>'block_contents','id'=>$sub->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $sub->iorder?$sub->iorder:0 }}" style="width:50px" />
                          </td>
                          <td>
                            {{ $sub->updated_at }}
                          </td>
                          
                          <td class="wrap-load-active" data-url="{{ route('admin.block_contents.load.status',['id'=>$sub->id]) }}">
                            @include('admin.components.load-change-status',['data'=>$sub,'type'=>'Khối nội dung'])
                          </td>
                          <td>
                            <a class="btn btn-sm btn-warning" data-toggle="tooltip" title="@lang('Edit')"
                              data-original-title="@lang('Edit')"
                          ...
                          <form action="{{ route(Request::segment(2) . '.destroy', $sub_child->id) }}" method="POST"
                            ...
                            <tr class="valign-middle bg-gray-light">
                              ...
                              <td>
                                ...
                                - - - - - - <input data-url="  {{ route('admin.loadOrderVeryModel',['table'=>'block_contents','id'=>$sub_child->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $sub_child->iorder?$sub_child->iorder:0 }}" style="width:50px" />
                              </td>
                              <td>
                                {{ $sub_child->updated_at }}
                              </td>
                              
                              <td class="wrap-load-active" data-url="{{ route('admin.block_contents.load.status',['id'=>$sub_child->id]) }}">
                                @include('admin.components.load-change-status',['data'=>$sub_child,'type'=>'Khối nội dung'])
                              </td>
                              <td>
                                <a class="btn btn-sm btn-warning" data-toggle="tooltip" title="@lang('Edit')"
                                  data-original-title="@lang('Edit')"
