                ...
                @if ($row->parent_id == 0 || $row->parent_id == null)
                  <form action="{{ route(Request::segment(2) . '.destroy', $row->id) }}" method="POST"
                    ...
                    <tr class="valign-middle">
                      ...
                      @if(isset($row->json_params->taxonomy_title->$lang))
                      <td>
                        {{ $row->json_params->taxonomy_title->$lang }}
                      </td>
                      @else
                      <td>
                        {{ $row->taxonomy_title }}
                      ...
                      <td class="wrap-load-hot" data-url="{{ route('admin.cms_posts.load.isFeatured',['id'=>$row->id]) }}">
                        @include('admin.components.load-change-is_featured',['data'=>$row,'type'=>'bản ghi'])
                      </td>
                      <td>
                        {{-- $row->iorder --}}
                        <input data-url="{{ route('admin.loadOrderVeryModel',['table'=>'cms_posts','id'=>$row->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $row->iorder?$row->iorder:0 }}" style="width:50px" />
                      </td>
                      <td>
                        {{ $row->updated_at }}
                      </td>
                      ...
                      <td class="wrap-load-active" data-url="{{ route('admin.cms_posts.load.status',['id'=>$row->id]) }}">
